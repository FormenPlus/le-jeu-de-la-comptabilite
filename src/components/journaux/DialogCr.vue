  ...
  <v-layout row justify-center>
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-text-field v-if="typeCpt === 'text'" label="Votre texte *" v-model="texte" autofocus></v-text-field>

            <v-row v-if="typeCpt === 'somme'">
              <div class="col-8"></div>
              <div class="col-4">
                <v-text-field type="number" label="La somme *" v-model="somme"
                              autofocus
                ></v-text-field>
              </div>
            </v-row>
            ...
            <v-row v-if="typeCpt === 'histo'">
              <div class="col-6">
                <p class="headline">{{ typeCpt === 'histo' ? 'Historique' : ''}}</p>
                <ul>
                  <li v-for="(ecriture, index) in _historiqueDuCpt(cpt)" :key="index">{{ ecriture }}</li>
                </ul>
              </div>
              <div class="col-6">
                <v-text-field type="number" label="La somme *" v-model="somme" autofocus
                ></v-text-field>
              </div>
            </v-row>
            ...
            <v-row v-if="typeCpt === 'text-somme'">
              <div class="col-8">
                <v-text-field label="Votre texte *" v-model="texte" autofocus
                ></v-text-field>
              </div>
              <div class="col-4">
                <v-text-field type="number" label="La somme *" v-model="somme"
        ...
        <v-card-actions class="mt-0" style="background-color: #eeeeee">
          <span class="caption red--text text--darken-2 font-weight-medium">* Champ(s) requis</span>
          <v-spacer></v-spacer>
          <v-btn color="black--text" text @click="closeDialog">Annuler</v-btn>
          <v-btn color="warning" @click="valider" :disabled="!isValid">Valider</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
  ...
  export default {
    ...
    methods: {
      valider() {
        ...
        this.$store.commit("dialogCr", false);
        this.resetInput();
      },
      closeDialog() {
        this.resetInput();
    ...
    computed: {
      ...
      dialog() {
        return this.$store.state.dialogCr;
      },
      isValid() {
        let valid = false;
